...
class MatchGame extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.onRunTimer, 1000)
  }
  ...
  onRunTimer = () => {
    const {secondsElapsed, timer} = this.state

    const isNotZero = secondsElapsed === timer
  ...
  generateDisplayImage = () => {
    const randomMainImageDetails = imagesList[Math.floor(Math.random() * 30)]

    return randomMainImageDetails
  }

  onCheckMatch = id => {
    const {displayImage} = this.state
    const randomImage = this.generateDisplayImage()
  ...
  renderGame = () => {
    ...
    const {selectedTabId, displayImage, secondsElapsed} = this.state

    const selectedTabImagesList = imagesList.filter(
    ...
    return (
      <div className="content-container">
        ...
        <ul className="images-list-container">
          {selectedTabImagesList.map(eachImage => (
            <Thumbnails
              eachImage={eachImage}
              onCheckMatch={this.onCheckMatch}
              key={eachImage.id}
            />
          ))}
        </ul>
  ...
  renderGameOver = () => {
    ...
    return (
      <div className="gameover-bg-container">
        ...
        />
        <p className="yourscore-title">YOUR SCORE</p>
  ...
  render() {
    const {score, isGamePlaying, secondsElapsed, timer} = this.state

    const secondsRemaining = timer - secondsElapsed
    return (
      <div className="bg-container">
        <Header score={score} secondsRemaining={secondsRemaining} />

        {isGamePlaying ? this.renderGame() : this.renderGameOver()}
      </div>
    )
  }
}
